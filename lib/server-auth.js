...
function loadSDK() {
    if (!sdk) {
        try {
            sdk = { ...require('firebase-admin/app'), ...require('firebase-admin/auth'), ...require('firebase-admin/firestore') };
        } catch (error) {
            const failure = new Error('SDK_LOAD_FAILED');
            failure.code = error.code || 'SDK_LOAD_FAILED';
            throw failure;
        }
    }
    return sdk;
...
function app() {
    const {getApps,getApp,initializeApp,cert}=loadSDK();
...
async function authorize(req,res) {
    ...
    try { app(); } catch (error) {
        const sdkFailure = error.message === 'SDK_LOAD_FAILED';
        json(res,500,{code:sdkFailure?'SDK_LOAD_FAILED':'FIREBASE_CONFIG_INVALID',error:sdkFailure?'تعذر تحميل مكتبات الخادم. أعد النشر مع تثبيت الاعتماديات وراجع Vercel Logs.':'إعدادات Firebase Admin ناقصة أو غير صالحة. راجع متغيرات البيئة في Vercel ثم أعد النشر.'});
        return null;
    }
    let token;
    try { token=await loadSDK().getAuth().verifyIdToken(match[1],true); }
...
module.exports={getDatabase:()=>loadSDK().getFirestore(app()),app,authorize,cors,json};
